...
if [ -f "/boot/rk3326-r36s-linux.dtb" ] || \
   [ -f "/boot/rk3326-r36h-linux.dtb" ] || \
   [ -f "/boot/rk3326-r45h-linux.dtb" ] || \
   [ -f "/boot/rk3326-rg351v-linux.dtb" ] || \
   [ -f "/boot/rk3326-rg351mp-linux.dtb" ] || \
   [ -f "/boot/rk3326-g350-linux.dtb" ] || \
   [ -f "/boot/rk3326-gameforce-linux.dtb" ] || \
   [ -f "/boot/rk3326-odroidgo3-linux.dtb" ]; then
...
# First boot
#
# Expand partition 3 and reboot.
# The actual filesystem expansion will continue after reboot.
# ------------------------------------------------------------
if [ ! -f /boot/doneit ]; then
  ...
  dialog --infobox \
    "EASYROMS partition expansion and conversion to exfat in process. The device will now reboot to continue the process..." \
...
# ------------------------------------------------------------
# Root filesystem and EASYROMS expansion
# ------------------------------------------------------------
(
  ...
  if [ ! -f "/boot/rk3326-r36s-linux.dtb" ] && \
     ...
     [ ! -f "/boot/rk3326-r45h-linux.dtb" ] && \
     [ ! -f "/boot/rk3326-rg351mp-linux.dtb" ] && \
     [ ! -f "/boot/rk3326-g350-linux.dtb" ]; then
  ...
  echo 100
      ...
      --title "Firstboot setup of dArkOSen" \
      --gauge "Expanding rootfs and exfat partitions. Please wait..." \
      $height $width 0 2>&1 > /dev/tty1
...
if [ "$exitcode" == "0" ]; then
  ...
  # OMG installation
  #
  # At this point the root filesystem has already been
  # expanded, so /etc has enough space for omg.service.
  # ----------------------------------------------------------
  omg_exitcode=0

  if [ -f "/boot/omg/bin/install.sh" ]; then

    dialog --infobox \
      "Installing OMG. Please wait..." \
      $height $width 2>&1 > /dev/tty1
  ...
  # Show result
  # ----------------------------------------------------------
  if [ "$omg_exitcode" -eq 0 ]; then

    dialog --infobox \
      "Completed expansion of root filesystem and EASYROMS partition and conversion to exfat. OMG has also been installed. The system will now reboot and load dArkOSen." \
      $height $width 2>&1 > /dev/tty1 | sleep 10

  else

    dialog --infobox \
      "dArkOS firstboot completed, but OMG installation failed. Please check /boot/omg/logs/omg-install.log. The system will now reboot and load dArkOSen." \
      $height $width 2>&1 > /dev/tty1 | sleep 10
  ...
  # ----------------------------------------------------------
  dialog --infobox \
    "EASYROMS partition expansion and conversion to exfat failed for an unknown reason. Please expand the partition using an alternative tool such as Minitool Partition Wizard. System will reboot and load dArkOSen now." \
    $height $width 2>&1 > /dev/tty1 | sleep 10
